...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function setupCardInteractions() {
    ...
    newsCards.forEach(card => {
      ...
      // 點擊卡片背面翻轉回正面 - 新增功能
      if (cardBack) {
        // 為背面頭部添加點擊事件，點擊時翻轉回正面
        const backHeader = cardBack.querySelector('.back-header');
        if (backHeader) {
  ...
  function setupTouchEvents() {
    newsCards.forEach(card => {
      const contentScroll = card.querySelector('.news-content-scroll');
      const cardInner = card.querySelector('.news-card-inner');
      const backHeader = card.querySelector('.back-header');
      ...
      // 為背面頭部添加觸控事件
      if (backHeader) {
        backHeader.addEventListener('touchstart', function(e) {
          startX = e.touches[0].clientX;
          startY = e.touches[0].clientY;
        }, { passive: true });
        
        backHeader.addEventListener('touchend', function(e) {
