...
function addMobileNavToggle() {
    // 檢查是否已存在切換按鈕
    if (document.querySelector('.mobile-nav-toggle.fixed-top')) return;
    
    console.log("Adding mobile nav toggle button");
    
    // 創建切換按鈕
    const toggleButton = document.createElement('button');
    toggleButton.className = 'mobile-nav-toggle fixed-top';
...
function toggleMobileNav(forceState) {
    ...
    if (newState) {
        ...
        // 更新按鈕圖標
        const icon = document.querySelector('.mobile-nav-toggle.fixed-top i');
        if (icon) {
...
function setupMobileNav() {
    ...
    // 獲取切換按鈕
    const toggleButton = document.querySelector('.mobile-nav-toggle.fixed-top');
    if (!toggleButton) {
...
function fixMobileSidebarScroll() {
    console.log("Fixing mobile sidebar scroll issues");
    ...
    // 移除側邊欄內的漢堡圖示
    const hamburgerButtons = header.querySelectorAll('button, .navbar-toggler, .mobile-nav-toggle:not(.fixed-top), .navmenu-toggle');
    hamburgerButtons.forEach(button => {
        if (!button.classList.contains('fixed-top')) {
    ...
    // 確保側邊欄內容有足夠的底部間距
    const navMenus = header.querySelectorAll('.navmenu, nav, .nav');
    navMenus.forEach(nav => {
        nav.style.paddingBottom = '30px';
        
        // 為最後一個導航項目添加額外的底部間距
        const lastItems = nav.querySelectorAll('li');
        if (lastItems.length > 0) {
            lastItems[lastItems.length - 1].style.marginBottom = '50px';
        }
    });
    ...
    if (socialLinks) {
        socialLinks.style.marginBottom = '50px';
    }
    
    // 動態添加樣式以確保側邊欄可滾動
    if (!document.getElementById('mobile-sidebar-fix-styles')) {
        const style = document.createElement('style');
        style.id = 'mobile-sidebar-fix-styles';
        style.textContent = `
            @media (max-width: 1199px) {
                ...
                /* 隱藏側邊欄內的漢堡圖示 */
                #header button:not(.fixed-top),
                #header .navbar-toggler,
                #header .mobile-nav-toggle:not(.fixed-top),
