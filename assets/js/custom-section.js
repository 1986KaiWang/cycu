...
/*function removeExtraHamburgerIcons() {
    ...
    // 移除側邊欄內的所有漢堡圖標
    const hamburgerIcons = sidebar.querySelectorAll('.bi-list, .fa-bars, .navbar-toggler, .mobile-nav-toggle');
    ...
    // 檢查並移除側邊欄內的漢堡按鈕
    const hamburgerButtons = sidebar.querySelectorAll('button, .toggle-button, .navbar-toggler');
    hamburgerButtons.forEach(button => {
        // 檢查按鈕是否包含漢堡圖標類
        const hasHamburgerIcon = button.querySelector('.bi-list, .fa-bars') || 
                                button.classList.contains('mobile-nav-toggle') ||
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (toggleButton) {
        toggleButton.addEventListener('click', function() {
            // 延遲執行以確保側邊欄已完全打開
            setTimeout(removeExtraHamburgerIcons, 100);
        });
    }
